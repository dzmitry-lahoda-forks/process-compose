...
func TestSystem_DisableEnableStopDeleteUpdateProcesses(t *testing.T) {
	...
	project := &types.Project{
		Processes:   map[string]types.ProcessConfig{"proc1": p1},
		ShellConfig: shell,
	}
	...
	waitForProcess(t, "proc1", runner)

	// Run new process
	added, err := runner.UpdateProcesses(&[]types.ProcessConfig{p2})

	if err != nil {
		t.Fatalf("UpdateProcesses failed: %v", err)
	}
	if len(added) != 1 || added["proc2"] != "ok" {
		t.Fatalf("Expected proc2 to be added, got: %v", added)
	}

	// Wait both processes to be running
	waitForProcess(t, "proc2", runner)
...
func waitForProcess(t *testing.T, proceName string, runner *ProjectRunner) {
	for _, name := range []string{proceName} {
		for attempts := 0; attempts < 200; attempts++ {
			st, err := runner.GetProcessState(name)
			if err == nil && st.IsRunning {
				break
